...
export default function Navbar({ activeFunction, darkMode, isDarkMode }) {
  ...
  return (
    <>
      <div className={`container bg-${!isDarkMode?"black":"white"} text-${color} w-10 h-screen mx-0 fixed gap-10 left-0 flex flex-col justify-center`} style={{
        ...
        <i className={`fa-regular fa-bell text-${color} justify-center self-center`}onClick={() => handleClick('Notification')}></i>
        ...
        <div className={`profilePhoto w-6 h-6 bg-${color} justify-center self-center`} onClick={() => handleClick('Profile')}></div>
      </div>
    </>
  );
}
